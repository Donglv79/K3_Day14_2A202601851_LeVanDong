...
class EvalResult:
    ...
    def overall_score(self) -> float:
        ...
        The retrieval metrics are intentionally excluded: they evaluate the
        retriever, while this score evaluates answer quality.
        """
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "You are an impartial AI judge. Score the answer from 0 to 1 for "
            "each criterion. Return valid JSON with criterion names as keys.\n\n"
            f"Question: {question}\n"
            f"Answer: {answer}\n"
            f"Rubric:\n{rubric_text}\n"
            "Return only JSON scores and optionally a short explanation."
        )
        ...
        try:
            ...
            fenced = re.search(r"```(?:json)?\s*(.*?)\s*```", json_text, re.DOTALL)
            if fenced:
                json_text = fenced.group(1)
            parsed = json.loads(json_text)
            parsed_scores = parsed.get("scores", parsed) if isinstance(parsed, dict) else {}
            scores = {
                criterion: max(0.0, min(1.0, float(parsed_scores[criterion])))
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        # Positional evidence is optional because a normal score_response call
        # contains one answer only. Detect it when paired-position scores are
        # supplied by an experiment.
        ...
        positional_bias = (
            len(position_differences) >= 2
            and sum(1 for difference in position_differences if difference > 0) / len(position_differences) >= 0.75
        )
        return {
...
class BenchmarkRunner:
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            # Keep the original pair, including its metadata and evidence.
            result.qa_pair = pair
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        A metric is considered regressed when its average decreases by more
        than 0.05 compared with the baseline.
        """
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        if list(scores.values()).count(scores[lowest_metric]) > 1:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        Build one row for each failure and pair it with the suggestion at the
        same index when one is available.
        """
        lines = [
            "| Failure ID | Type | Root Cause | Suggested Fix | Status |",
            "|------------|------|------------|---------------|--------|",
        ...
        for index, failure in enumerate(failures, start=1):
            ...
            suggestion = suggestions[index - 1] if index <= len(suggestions) else "Review failure trace"
            lines.append(
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination", 0):
            suggestions.append(
                "Add a grounding check to reject claims that are not supported by retrieved context"
            )
        if categories.get("irrelevant", 0) or categories.get("off_topic", 0):
            suggestions.append(
                "Improve prompt instructions so the agent answers the question directly and stays in scope"
            )
        if categories.get("incomplete", 0):
            suggestions.append(
                "Increase useful context coverage and add examples of complete answers"
            )
        if categories.get("refusal", 0):
            suggestions.append(
                "Review guardrails so the agent refuses only unsafe or out-of-scope requests"
            )

        defaults = [
            "Inspect retrieved chunks for missing or noisy evidence",
            "Add regression tests for the failed questions before the next release",
            "Review the failure traces and update the golden dataset with edge cases",
        ]
